    ...
    if run_button:
        ...
        if chosen_content:
            with st.spinner("🔄 연산 중..."):
                ...
                [특정 제약 조건]
                - 현재 계좌 상태에 존재하는 종목코드 '278530' (KODEX 200TR)은 종목코드 '294400' (KIWOOM 200TR)과 실질적으로 동일한 추종 자산입니다.
                - 목표 비중 편차 연산 및 비교 시에는 '278530'과 '294400'의 현재 비중 합계를 Investment_Target의 '294400 KIWOOM 200TR' 비중(7.6%)과 매칭하여 비교하십시오.
                - **중요**: 추가 매수 추천('buy_recommendation')을 산출할 때에는 절대로 '278530' 종목을 신규 매수하지 마십시오. 부족한 비중 분량은 오직 '294400' 종목만 매수하도록 수량을 집계해야 합니다.
                - 매수 후 업데이트된 계좌 내역('updated_account_lines')을 기술할 때 기존 '278530'의 주수와 비중은 그대로 유지하고, 추가 매수된 분량만큼 '294400'의 주수와 비중을 늘려 반영하십시오.
